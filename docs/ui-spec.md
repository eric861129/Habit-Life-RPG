...
`Habit Life RPG` 的視覺語言應該讓使用者感覺自己正在進入一個輕量 RPG 介面，而不是一般待辦清單。畫面可以使用 `Quest`、`Hero`、`Guild Rank`、`Reward` 等 RPG 呈現語彙，但資料欄位、API、測試與程式碼命名仍維持 `habit` / `habits` / `habit_id`。

核心感受：

- 深色 RPG HUD，帶有任務板、角色面板與戰利品回饋感。
- 霓虹綠強調色，代表打卡成功、能量與升級回饋。
- 金色作為 reward、rank、gold 的輔助色，避免畫面只剩單一色調。
...
| Token | 色碼 | 用途 |
| :--- | :--- | :--- |
| `--color-primary` | `#2D3436` | App 背景與主要深色區域 |
| `--color-accent` | `#00CEC9` | 主要行動、成功狀態、進度條 |
| `--color-surface` | `#1E272E` | Hero 面板、Quest 卡片、底部導覽 |
| `--color-surface-muted` | `#27343A` | 次要容器與 hover 類狀態 |
| `--color-text` | `#F5F7FA` | 主要文字 |
| `--color-muted` | `#AEB8BE` | 次要文字、說明文字 |
| `--color-gold` | `#FDCB6E` | 金幣與獎勵 |
| `--color-danger` | `#FF7675` | 錯誤、HP 風險、失敗狀態 |
...
- RPG 裝飾只能用細邊框、角標、rank 標籤與 reward chip，不加入複雜背景圖或難以維護的美術素材。

## 5. 字體

- 使用系統字體堆疊：`system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`。
...
第三章不導入 Phaser、Three.js 或遊戲引擎。本章採用 `Game UI Frontend` 的思路，把畫面先設計成 RPG HUD：

- 玩家動詞：完成 habit、打卡領獎、累積 EXP、避免 HP 扣減。
- 核心循環：看見 Hero Status -> 選擇 Quest -> 打卡 -> 領取 reward -> 檢查是否升級。
- UI 表層：用 Quest Log、Guild Rank、Reward chip、Level gate 呈現遊戲感。
...
畫面中段顯示今日可打卡 habit。UI 可稱為 Quest Log，但底層仍是 habit 清單。
...
| Habit item | Ready | Quest 卡片、accent 邊線、rank 標籤 |
| Habit item | Done | 已完成狀態，保留 reward 但降低強調 |
